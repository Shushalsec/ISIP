...
if __name__== '__main__':
    ...
    # Initial conditions
    img = plt.imread(files_names[0])
    simple_img = imgsimplify(img, c_in, window_in)

    # Training information
    train_info = np.zeros((len(training_pictures)+1, window_in, window_in, 3))
    for j in range(len(training_pictures)):
        img_train = plt.imread(training_pictures[j])
    ...
    for i in range(1,len(files_names)):
        ...
        img_next = plt.imread(files_names[i])
        simple_imgnext = imgsimplify(img_next, c_in, window_next)

        # Compute NCC and take the pixel with the highest value
        NCC_vals = match_template(simple_imgnext, simple_img, pad_input=True, mode="edge")
        NCC_onedim = np.sum(NCC_vals, axis=2)
        ...
        NCC_vals_ampl = match_template(simple_imgnext, template_amplified, pad_input=True, mode="edge")
        NCC_onedim_ampl = np.sum(NCC_vals_ampl, axis=2)
        ...
        if NCC_max < threshold:
            ...
            for training in train_info:
                NCC_training = match_template(simple_imgnext, training, pad_input=True, mode="edge")
                NCC_training_onedim = np.sum(NCC_training, axis=2)
